...
$.fn.customSelect = function (options) {
	...
	return this.each(function () {
		...
		if (settings.method == 'refresh') {
			...
			if (!$wrapper[0].hasAttribute('data-search')) $wrapper.find('.custom-select-container').attr('data-selected', $input.val() || data.placeholder.toLowerCase());
		}
		...
		if (!settings.method) {
			...
			$wrapper.on('click', '.custom-select-value', function () {
				customSelectArray.forEach(function (item) {
					if (!$wrapper.is(item)) item.removeClass('custom-select-open').find('.custom-select-dropdown').hide();
				});
			...
			$wrapper.on('click', '.custom-select-dropdown li', function () {
				...
				$input.val($(this).data('text')).data('text', $(this).data('text')).change();

				if (!$wrapper[0].hasAttribute('data-search')) $container.attr('data-selected', $(this).data('text'));
