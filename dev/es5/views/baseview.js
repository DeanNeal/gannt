...
var BaseView = Backbone.View.extend({
    ...
    initialize: function (params) {
        ...
        if (!this.collection && this.parent) this.collection = this.parent.collection;
    ...
    renderNestedView: function (view, targetElement) {
        var el = this.$el;
        if (targetElement) el = typeof targetElement === 'string' ? this.$el.find(targetElement) : targetElement;
    ...
    render: function (redraw) {
        ...
        if (!this.isRendered) {
            ...
            if (html) {
                ...
                if ($(el).find('.template').length) this.contentInternal = $(el).find('.template').children();else this.contentInternal = el.firstElementChild;
    ...
    changeStage: function (params) {
        if (params.stagesArray[0]) {
            this.beforeChangeStage(params).then((function (data) {
                ...
                if (this.currentStage !== params.stagesArray[0]) {
                    // if current stage is already rendered and next stage doesn't exist
